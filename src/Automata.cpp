...
void Automata::on() {
    if (state == Automata::OFF) {
        state = Automata::WAIT;
    } else throw "Already on";
}

void Automata::off() {
    if (state == Automata::WAIT) {
        state = Automata::OFF;
    } else throw "Can't off";
}
